...
class AuthSevices {
  ...
  Future<UserModel> register(SignUpFromModel data) async {
    try {
      final res = await http.post(
      ...
      if (res.statusCode == 200) {
        ...
        await storeCrendtialTolocal(user);
  ...
  Future<UserModel> login(SignInFromModel data) async {
    try {
      final res = await http.post(
  ...
  Future<void> storeCrendtialTolocal(UserModel user) async {
    try {
  ...
  Future<SignInFromModel> getCredenstsialFromLocal() async {
    try {
      const storage = FlutterSecureStorage();
      ...
      if (values['email'] == null || values['password'] == null) {
        ...
        final SignInFromModel data = SignInFromModel(
          email: values['email'],
